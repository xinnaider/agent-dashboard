//! Tiny helper binary that injects keystrokes into another process's console.
//! Usage: send-keys <PID> <TEXT>
//!
//! Uses AttachConsole + WriteConsoleInputW so it works even when the target
//! process is in a different console window.  Designed to be spawned with
//! CREATE_NO_WINDOW so it never pops up a visible window.
...
fn main() {
    ...
    unsafe {
        ...
        // Send each character as key-down + key-up
...
fn main() {
    ...
    if let Ok(mut f) = fs::OpenOptions::new().write(true).open(&stdin_path) {
